...
def main(args):
    ...
    WIN_SAMPLES = int(0.010 * fs)                             # 10 ms → 300 samples
    ...
    model     = ResNet1D([2, 2, 2, 2], in_ch=train_dataset.n_chan).to(device)
    ...
    if args.checkpoint_dir and (args.resume_epoch is not None):
        ckpt_path = os.path.join(
            args.checkpoint_dir,
            f"checkpoint_epoch_{args.resume_epoch}.pth"
        )
